...
const AnimatedList: React.FC<AnimatedListProps> = ({
  ...
  return (
    <div className={`relative w-[500px] ${className}`}>
      <div
        ref={listRef}
        className={`max-h-[900px] overflow-y-auto p-4 ${
          displayScrollbar
          ...
          >
            <Card className="shadow-md shadow-white">
              ...
              <div className="px-4 flex flex-col  py-2">
                <div className="flex max-md:text-sm gap-2 md:gap-4 items-center  text-center">
                  <CalendarIcon className="max-md:size-4" />
                  <p>Date: {item.date}</p>
                </div>
                <div className="mt-5 space-y-2">
                  {item.desc.map((d, i) => (
                    <p key={i} className="text-white max-md:text-xs  ">
                      {" "}
                      {" > "} {d}
                    </p>
                  ))}
